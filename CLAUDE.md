...
An Activity with only a fuzzy `timeLabel` (no `startAt`/`endAt` — see `data-model.html`) still needs somewhere to carry which date it's on: set its `date` field alongside `timeLabel` in that case (`trip-model.js`'s `resolveActivityDate`).
...
### The five components `@material/web` doesn't have

`@material/web` has no Card, Side Sheet, Date Picker, Time Picker, or App Bar component (confirmed against the library's component list, including `labs/`; it's also in maintenance mode with no active maintainers). Rather than reinvent them as bespoke-CSS lookalikes, each is approximated from real `md-*` primitives plus the minimum structural CSS to lay them out — this is the one deliberate exception to "no custom CSS," and it's confined to `styles.css`'s `day-surface`/`detail-side-sheet`/`date-picker-*`/`time-picker*`/`app-bar` rules:
...
- **Side sheet** — `<detail-side-sheet>` (`docs/js/side-sheet.js`), built from `md-elevation`, `md-icon-button`, `md-divider`, and `md-list`. It's the **modal** variant of the [M3 side-sheet spec](https://m3.material.io/components/side-sheets/overview) (the spec's other variant, standard, sits inline beside content with no scrim — not used here): anchored to the trailing (right) edge, full height and edge-to-edge (not the spec's optional detached/margined treatment), a scrim behind it, dismissed via the scrim, Escape, or the close button. `styles.css`'s `detail-side-sheet` rules mirror the spec's own numbers directly — `surface-container-low` container color, 16dp (`--md-sys-shape-corner-large`) corner radius on the two inner corners only, 400dp max-width, 24dp container padding, 12dp gap between header elements.
- **Date picker** — `docs/js/date-picker.js`'s `renderDatePicker`, a docked month-grid calendar shown in `#date-picker-dialog` (a real `md-dialog`, following the same pattern as `#leg-dialog`), built from `md-icon-button` for month navigation plus plain `<button>` day cells; only dates that land on an actual trip Day are selectable. Reused for both the trip page's "Jump to a day" control and every date field an edit form opens (`edit.js`'s `dateTimeFieldRow`).
- **Time picker** — `docs/js/time-picker.js`'s `renderTimePicker`, the M3 spec's **Input** variant (hour/minute text fields plus an `md-tabs` AM/PM toggle — this codebase's existing "choose one of a few options" convention, not `labs/segmentedbutton*`, which would collide with `all.js`'s own bundle over the CDN — see the file's own note) rather than the spec's Dial variant, shown in `#time-picker-dialog`; opened from the same edit-form date+time fields as the date picker above, always as its field's other half.
- **App bar** — `#app-bar` (styled in `styles.css`, wired in `app.js`), the sticky bar above the day list holding the "Jump to a day" and Filters icon buttons; sized to the [M3 app bar spec](https://m3.material.io/components/app-bars/overview)'s Small variant (64dp — renamed from "Top app bar" in M3's May 2025 Expressive update, which also added a Search app bar variant and flexible Medium/Large variants, none of which apply here) and, per that spec, flat while it's the page's own leading edge and only shadowed once content has scrolled in underneath it (`app.js`'s `updateAppBarElevation`, toggling `.is-elevated`) rather than shadowed unconditionally.
