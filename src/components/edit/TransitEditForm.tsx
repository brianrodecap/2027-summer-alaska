  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" spacing={2}>
        <TextField label="From" value={form.fromLabel} onChange={(e) => onChange({ ...form, fromLabel: e.target.value })} fullWidth />
        <TextField label="To" value={form.toLabel} onChange={(e) => onChange({ ...form, toLabel: e.target.value })} fullWidth />
      </Stack>
      <DateTimeFieldPair
      ...
      {hasRoute && (
        <Typography variant="body2" color="text.secondary">
          Arrival is computed from the selected route's own drive times, updated live as it's picked.
        </Typography>
      )}
      <TextField
        select
