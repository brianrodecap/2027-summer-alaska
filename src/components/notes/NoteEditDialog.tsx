...
const KIND_LABEL: Record<NoteKind, string> = { warning: 'Alert', info: 'Info', footnote: 'Footnote' };
const KIND_ICON: Record<NoteKind, typeof WarningIcon> = { warning: WarningIcon, info: InfoIcon, footnote: NotesIcon };
...
export function NoteEditDialog({
  ...
  return (
    <Dialog open onClose={onClose} fullWidth maxWidth="xs">
      <DialogTitle>{target.mode === 'create' ? `Add ${KIND_LABEL[kind].toLowerCase()}` : 'Edit note'}</DialogTitle>
      <DialogContent>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          About {about}
        </Typography>
        <ToggleButtonGroup value={kind} exclusive fullWidth onChange={(_, v: NoteKind | null) => v && setKind(v)} sx={{ mb: 2 }}>
      ...
      <DialogActions sx={{ justifyContent: onDelete ? 'space-between' : 'flex-end', px: 3, pb: 2 }}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          <Button variant="contained" disabled={!text.trim()} onClick={() => onSave(kind, text.trim())}>
            Save
          </Button>
        </Stack>
      </DialogActions>
