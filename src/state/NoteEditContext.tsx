...
export function NoteEditProvider({ children }: { children: ReactNode }) {
  ...
  const openNoteCreate = useCallback(
    (ref: Ref, kind: NoteKind) => setTarget({ mode: 'create', ref, kind }),
    [],
  );
  const openNoteEdit = useCallback((note: Note) => setTarget({ mode: 'edit', note }), []);
  const closeNoteEdit = useCallback(() => setTarget(null), []);
